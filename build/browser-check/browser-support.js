...
const supportedBrowserRegexp = {
  ...
  [BROWSER_TYPES.WEB]: supportedWebBrowsers,
};
const browserTargets = {
  default: 'unsupported-browser.html',
};
...
const isBrowserSupported = (browserType, userAgent) =>
  supportedBrowserRegexp[browserType].test(userAgent);
...
module.exports.redirectIfUnsupported = (userAgent) => {
  ...
  // Test if Supported by given deviceType
  const isSupported = isBrowserSupported(browserType, userAgent);

  if (!isSupported) {
    // Execute Redirect
    // If we tested this next method, the browser would redirect, killing the test
